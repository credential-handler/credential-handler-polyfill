...
export function createInteractRequest({
  ...
  try {
    ...
    throw new Error('"interactionUrl" must be a valid "https:" URL.');
  }
  if(parsed.protocol !== 'https:') {
    throw new Error('"interactionUrl" must be an "https:" URL.');
  }
  if(recommendedHandlerOrigins !== undefined &&
    !Array.isArray(recommendedHandlerOrigins)) {
    throw new TypeError('"recommendedHandlerOrigins" must be an array.');
  }
